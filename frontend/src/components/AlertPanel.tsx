...
const severityConfig: Record<string, { bar: string; badge: string; label: string }> = {
  ...
  INFO:     { bar: 'bg-blue-400', badge: 'badge-info', label: 'Info' },
  SYSTEM:   { bar: 'bg-gray-400', badge: 'badge-system', label: 'System' },
};
    ...
    <div className="card flex flex-col h-full">
      <div className="card-header flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{alerts.length} notification{alerts.length !== 1 ? 's' : ''}</p>
        </div>
          ...
          <div className="divide-y divide-gray-100">
            {alerts.map((alert) => {
              ...
              return (
                <div
                  key={alert.id}
                  className={`flex gap-3 px-4 py-3 hover:bg-gray-50 transition-colors ${alert.is_read ? 'opacity-60' : ''}`}
                >
                  {/* Severity bar */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {alert.significance !== undefined && (
                        <span className="text-xs text-gray-400">{alert.significance}%</span>
                      )}
                    </div>
                    ...
                    {(alert.frame_url || alert.frame_base64) && (
                      <div className="mb-2 rounded-md overflow-hidden border border-gray-200">
                        ...
                        />
                        <div className="px-2 py-1 text-xs text-gray-400 text-center bg-gray-50 border-t border-gray-100">
                          Supporting evidence
                        </div>
                      </div>
                    )}
