...
const AppShell: React.FC = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: '#FAF9F7' }}>
      {/* Sidebar — warm cream, light */}
      <aside
        className="w-52 flex flex-col flex-shrink-0"
        style={{ background: '#F2EFE9', borderRight: '1px solid #E8E3DB' }}
      ...
      >
        {/* Logo */}
        <div className="px-4 pt-6 pb-5 flex flex-col items-center" style={{ borderBottom: '1px solid #E8E3DB' }}>
          {/* Animated eye logo */}
          <svg ref={svgRef} width="132" height="98" viewBox="0 0 110 82" fill="none" xmlns="http://www.w3.org/2000/svg">
            <defs>
            ...
            <g clipPath="url(#eye-lid-clip)">
              ...
              <rect x="-5" y="-5" width="120" height="55" fill="#F2EFE9" className="eyelid-top"/>
              {/* Lower eyelid — rises from bottom on blink */}
              <rect x="-5" y="45" width="120" height="42" fill="#F2EFE9" className="eyelid-bottom"/>
            </g>
          ...
          <span className="font-display font-bold text-3xl mt-1" style={{ color: '#1A1714', letterSpacing: '-0.04em' }}>
            ThirdEye
          </span>
        </div>
        ...
        {(activeCameras > 0 || (stats?.critical_alerts ?? 0) > 0) && (
          <div className="px-5 py-2.5 space-y-1" style={{ borderBottom: '1px solid #E8E3DB' }}>
            {activeCameras > 0 && (
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-green-500 flex-shrink-0" />
                <span className="text-[11px]" style={{ color: '#8C837A' }}>
            ...
            {(stats?.critical_alerts ?? 0) > 0 && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px]" style={{ color: '#7C2D2D' }}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                style={{
                  color: isActive ? '#1A1714' : '#8C837A',
                  background: isActive ? '#E8E3DB' : 'transparent',
                }}
                onMouseEnter={(e) => {
                  if (!isActive) {
                    (e.currentTarget as HTMLElement).style.color = '#1A1714';
                    (e.currentTarget as HTMLElement).style.background = 'rgba(232,227,219,0.5)';
                  }
                }}
                onMouseLeave={(e) => {
                  if (!isActive) {
                    (e.currentTarget as HTMLElement).style.color = '#8C837A';
        ...
        <div className="px-5 py-4" style={{ borderTop: '1px solid #E8E3DB' }}>
          {user && (
            <p className="text-[11px] truncate mb-2" style={{ color: '#B5ADA6' }} title={user.email}>
              {user.email}
            </p>
          )}
          <button
            ...
            onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.color = '#8C837A')}
          >
            Sign out
          </button>
        </div>
